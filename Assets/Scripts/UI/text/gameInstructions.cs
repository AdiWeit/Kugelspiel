...
public class gameInstructions : MonoBehaviour
{
    ...
    public void levelDone(int pLevel, bool random) {
      ...
      if (SystemInfo.deviceType == DeviceType.Handheld) {
        ...
        if ((random && pLevel == 5) || (!random && pLevel == 13)) instructions.text = "Don't let the big gray one block your hole! It has to come in last! (if there are multiple ones on screen, one blocking the hole is enough.)";
        if ((random && pLevel == 6) || (!random && pLevel == 14)) instructions.text = "Blue marbles roll faster!";
        if ((random && pLevel == 7) || (!random && pLevel == 20)) instructions.text = "Orange marbles bounce, so they can overcome the borders. Always have an eye on them! Also, you do not need to get them into the hole!";
    }
